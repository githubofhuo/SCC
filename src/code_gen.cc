...
{
  if (paramType->IsInteger() || paramType->ToPointer() || paramType->ToArray())
  {
...
std::string Generator::ConsLabel(Constant *cons)
{
  ...
  else if (cons->Type()->IsFloat())
  {
    ...
    long val = (width == 4) ? *reinterpret_cast<int *>(&valss) : *reinterpret_cast<long *>(&valsd);
...
static const char *GetLoad(int width, bool flt = false)
{
  ...
  case 2:
    return "movzwq";
  case 4:
    return !flt ? "movl" : "movss";
  case 8:
    return !flt ? "movq" : "movsd";
  default:
    assert(false);
    return nullptr;
  }
...
void Generator::VisitBinaryOp(BinaryOp *binary)
{
  ...
  {
  case '*':
    return GenMulOp(width, flt, sign);
  case '/':
  case '%':
    return GenDivOp(flt, sign, width, op);
  case '<':
...
// FIXME: for combined assignment operator, if the rvalue expr
...
void Generator::GenCastOp(UnaryOp *cast)
{
  ...
  else if (srcType->ToPointer() || srcType->ToFunc() || srcType->ToArray())
  {
    // Handle bool
    if (desType->IsBool())
    {

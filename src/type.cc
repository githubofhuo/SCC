...
#define NEW_TYPE(tag) \
  ...
  static auto llongType = NEW_TYPE(T_LLONG) static auto ullongType = NEW_TYPE(T_UNSIGNED | T_LLONG);
...
int ArithmType::Spec2Tag(int spec)
{
  ...
  spec &= ~T_SIGNED;
  if ((spec & T_SHORT) || (spec & T_LONG) || (spec & T_LLONG))
  {
    spec &= ~T_INT;
  }
  return spec;
...
bool PointerType::Compatible(const Type &other) const
{
  ...
  // FIXME: cannot loose compatible constraints
...
bool ArrayType::Compatible(const Type &other) const
{
  ...
  if (!otherArray)
    return false;
  if (!derived_->Compatible(*otherArray->derived_))
    return false;
...
bool FuncType::Compatible(const Type &other) const
{
  ...
  if (!otherFunc)
    return false;
  // : do we need to check the type of return value when deciding

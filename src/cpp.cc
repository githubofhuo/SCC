...
void Preprocessor::Process(TokenSequence &os)
{
    ...
    auto sccHeaderFile = SearchFile("scc.h", true, false, filename_in);
    if (!sccHeaderFile)
        Error("can't find header files, try reinstall scc");
    IncludeFile(is, sccHeaderFile);
    Expand(os, is);
    Finalize(os);
}
...
void Preprocessor::ParseEndif(TokenSequence ls)
{
    ...
    while (!ppCondStack_.empty())
    {
        ...
        if (top.tag_ == Token::PP_IF || top.tag_ == Token::PP_IFDEF || top.tag_ == Token::PP_IFNDEF)
        {
            return;
        }
    }
                                      ...
                                      const std::string &curPath)
{
    ...
    for (; iter != searchPaths_.end(); ++iter)
    {
        ...
        if (dd == -1) // : or ensure it before preprocessing
            continue;
...
void Preprocessor::Init()
{
    ...
    AddSearchPath("/usr/local/scc/include/");
